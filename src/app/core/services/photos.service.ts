...
export class PhotosService {
    ...
    private profilePicturesPath: string = '/profiles';
    ...
    // Upload photo to Firebase storage and then saving photo info to database
    uploadProfilePicture(uploadPhoto: Photo): Promise<any> {
        return new Promise((resolve, reject) => {
            let storageRef = firebase.storage().ref();
            let photoNewName = this.generateName(uploadPhoto.file.name);

            this.uploadTask = storageRef.child(`${this.profilePicturesPath}/${photoNewName}`).put(uploadPhoto.file);
